...
@app.get("/")
def read_root():
    return {"status": "Servidor HISTORIAL_CLINICO_NUBE activo", "version": "1.2 - Fix Validaciones"}
...
def create_declaracion(declaracion: schemas.DeclaracionJuradaCreate, db: Session = Depends(get_db)):
    ...
    if not db_paciente:
        raise HTTPException(status_code=404, detail="Error: El paciente no existe")
