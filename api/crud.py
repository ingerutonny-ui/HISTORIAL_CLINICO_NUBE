...
def create_filiacion(db: Session, filiacion: schemas.FiliacionCreate):
    try:
        ...
        # Filtro de seguridad: solo guarda lo que existe en la tabla DeclaracionJurada
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Error Crítico P1: {str(e)}")
...
def create_antecedentes(db: Session, antecedentes: schemas.AntecedentesCreate):
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Error Crítico P2: {str(e)}")
...
def create_habitos(db: Session, habitos: schemas.HabitosP3Create):
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Error Crítico P3: {str(e)}")
