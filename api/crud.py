...
def create_filiacion(db: Session, filiacion: schemas.FiliacionCreate):
    try:
        ...
        # Convertimos todo a String para que coincida con models.py
        ...
        db.rollback()
        # Este mensaje saldrá en tus logs de Render si algo falla internamente
...
def create_antecedentes(db: Session, antecedentes: schemas.AntecedentesCreate):
    try:
        ...
        db_ant = models.AntecedentesP2(paciente_id=int(data.get("paciente_id")))
        db.add(db_ant)
        db.commit()
        db.refresh(db_ant)
        return db_ant
...
def create_habitos(db: Session, habitos: schemas.HabitosP3Create):
    try:
        ...
        db_hab = models.HabitosRiesgosP3(paciente_id=int(data.get("paciente_id")))
        db.add(db_hab)
        db.commit()
        db.refresh(db_hab)
        return db_hab
