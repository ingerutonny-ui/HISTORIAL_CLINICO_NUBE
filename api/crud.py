...
def create_filiacion(db: Session, filiacion: schemas.FiliacionCreate):
    try:
        ...
        # Filtra solo los campos que existen en el modelo de BD
        db_filiacion = models.DeclaracionJurada(**{k: str(v) if v is not None else "" for k, v in data.items() if k in columnas})
        db.add(db_filiacion)
        db.commit()
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Error en BD P1: {str(e)}")
...
def create_antecedentes(db: Session, antecedentes: schemas.AntecedentesCreate):
    try:
        ...
        db_ant = models.AntecedentesP2(**{k: str(v) for k, v in data.items() if k in columnas})
        db.add(db_ant)
        db.commit()
        db.refresh(db_ant)
        return db_ant
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Error en BD P2: {str(e)}")
...
def create_habitos(db: Session, habitos: schemas.HabitosP3Create):
    try:
        ...
        db_hab = models.HabitosRiesgosP3(**{k: str(v) for k, v in data.items() if k in columnas})
        db.add(db_hab)
        db.commit()
        db.refresh(db_hab)
        return db_hab
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Error en BD P3: {str(e)}")
